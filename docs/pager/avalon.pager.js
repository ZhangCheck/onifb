...
define(["avalon",
        ...
        var vmodel = avalon.define(data.pagerId, function (vm) {
            ...
            vm.jumpPage = function (event, page) {
                ...
                if (enabled && page !== vm.currentPage) {
                    ...
                    vm.onJump.call(element, event, vm, vm.currentPage,oldValue)
                    efficientChangePages(vm.pages, getPages(vm))
                    if(options.useInnerRequest&&options.parentVMId!=="$"&&options.parentInterface)
                    {
                        if(options.argFormat&&typeof options.argFormat==="function")
                        {
                            options.parentInterface.data=options.argFormat.call(element,options.parentInterface.data||{},vmodel.currentPage||0,vmodel.perPages||options.perPages);
                        }
                        avalon.vmodels[options.parentVMId].changePage(options.parentInterface);
                    }
                }
            },
            ...
            vm.changeCurrentPage = function (e, value) {
                ...
                var oldValue = vmodel.currentPage
                vmodel.currentPage = value
                vmodel.pages = getPages(vmodel)
                vmodel.onJump.call(element, e, vm, vmodel.currentPage, oldPage);
            }
            vm.pages = []
    ...
    widget.defaults = avalon.mix({
        parentVMId:"$",//若pager被其他组件使用，该属性用来存放引用组件的vmId
        useInnerRequest:false,
        parentInterface:null,//若pager被其他组件使用，该属性用来存放引用组件的interface
        argFormat:null,///若pager被其他组件使用，该属性用来存放引用组件的请求参数格式化方法
        ...
        firstPage: 1, //@config {Number} 当前可显示的最小页码，不能小于1
         ...
         */
        onJump: function (e, vm, newPage, oldPage) {
