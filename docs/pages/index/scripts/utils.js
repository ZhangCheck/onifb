...
define([], function(){
    var utils = {
        ...
        prepareExampleDirectory: function(pagesVM, models, widgetId){
            ...
            var exampleObj = {},widgetExs = exampleObj[widgetId] = avalon.mix( true, {}, models.examples[widgetId] );

            for(var exId in widgetExs){
                var exampleId = widgetExs[exId].match(/ex\d*(?=\.html)/)[0],exHref = "#!/widgets/?widgetId=" + widgetId + "&ex=" + exampleId;
        ...
        prepareApi: function(currentApi, pagesVM, apisVm, apis){
            ...
            if (typeof currentApi !== "undefined") {
                ...
                pagesVM.currentAnchor = "fb架构图"
            }

            pagesVM.directorys = apisObj
        },

        prepareDoc: function(curDoc,pagesVM,docVm,docs){
            var firstSrc, src;

            pagesVM.activeNav = "文档"
            pagesVM.listSrc = "pages/index/views/doc/list.html"

            // 确定active anchor
            if(curDoc===void(0)) curDoc = "概要";
            pagesVM.currentAnchor = curDoc
           ...
           docVm.docSrc = src
            ...
            // 添加href
            var docsObj = avalon.mix(true, {}, docs)

            for (var groupIndex in docsObj) {
                var apiGroup = docsObj[groupIndex]

                for (var apiIndex in apiGroup) {
                    apiGroup[apiIndex] = {
                        content: apiGroup[apiIndex],
                        href: "#!/doc?doc=" + apiIndex
                    }
                }
            }
            ...
            pagesVM.directorys = docsObj
        },

        prepareTools:function(curDoc,pagesVM,docVm,docs){
            var firstSrc, src;

            pagesVM.activeNav = "工具"
            pagesVM.listSrc = "pages/index/views/tools/list.html"

            // 确定active anchor
            if(curDoc===void(0)) curDoc = "WebStorm自动提示";
            pagesVM.currentAnchor = curDoc
