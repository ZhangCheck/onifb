...
define(['avalon',
    ...
    function setItemLabel(items) {
        avalon.each(items, function(i, item) {
            ...
            item.label = ['<i class="oni-icon ', item.iconClass, '">','','</i>', item.label].join('');
            item.titleValue = ['<i class="oni-icon ', item.iconClass, '">','', '</i>', item.titleValue].join('');
        });
        return items;
    }
