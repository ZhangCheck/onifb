		...
		avalon.mix(vm, {
			...
			asyncChildNodes: function(leaf, callback, filter) {
				...
				vm.excute("async", {
					...
					if(async.contentType=="application/json"){
						var dataParam = {data: JSON.stringify(data)}
					}else{
						var dataParam = {data: data}
					}
					var ajaxParam = avalon.mix(dataParam, async);
					avalon.ajax(ajaxParam).done(function(res) {
