    ...
    var widget = avalon.ui.tree = function(element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.treeId, function(vm) {
            ...
            vm.computeIconClass = function(leaf) {
                return (leaf.iconSkin ? leaf.iconSkin + "_" : "") + "oni-icon-" + (leaf.isParent ? vm.hasClassOpen(leaf, "ignoreNoline") ? "folder-open" : "folder-close" : "docu")
            }
             ...
             */
            vm.getNodesByFilter = function (filter, isSingle, startLeaf, options) {
            ...
            /**
             * add by wolf.j on 2015-11-25
             * @interface 根据值过滤查询，显示满足条件的节点及父节点，隐藏其他节点。***前序遍历节点***
             * @param value 模糊匹配的值，为空时则显示全部。
             */
            vm.searchNodes = function (value,startLeaf,hiddenedArray) {

                function searchChildrenNodes(value, startLeaf, parentHidden,hiddenedArray) {
                    var currentHidden = {hidden: true};//当前节点是否隐藏
                    //子
                    if (startLeaf.children && startLeaf.children.length > 0) {
                        for (var i = 0; i < startLeaf.children.length; i++) {
                            searchChildrenNodes(value, startLeaf.children[i], currentHidden,hiddenedArray);
                        }
                    }
                    //父
                    if (startLeaf.name.indexOf(value) >= 0 || !currentHidden.hidden) {
                        startLeaf.isHidden = false;
                        parentHidden.hidden = false;
                        parentHidden.open = true;
                    } else {
                        startLeaf.isHidden = true;
                        hiddenedArray.push(startLeaf);
                    }
                }
                ...
                if (vm.children && vm.children.length > 0) {
                    for (var i = 0; i < vm.children.length; i++) {
                        searchChildrenNodes(value, vm.children[i], currentHidden,hiddenedArray);
                    }
                    vm.children[0].isHidden=false;
                }
                return currentHidden.hidden;//true:表示没有查到
            }
            /**
             *  @interface 显示指定隐藏的node.
             * @param hiddenedArray
             */
            vm.clearSearchNodes = function (hiddenedArray) {
                if(hiddenedArray && hiddenedArray.length>0) {
                    for (var r = 0; r < hiddenedArray.length; r++) {
                        var obj = hiddenedArray[r];
                        obj.isHidden = false;
                    }
                }
            };
