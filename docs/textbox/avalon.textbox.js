...
define(["./avalon.suggest", "text!./avalon.textbox.html","css!../chameleon/oniui-common.css", "css!./avalon.textbox.css"], function(avalon, sourceHTML) {
    ...
    var widget = avalon.ui.textbox = function(element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.textboxId, function(vm) {
            ...
            vm.clearBtnHeight=0;
            vm.clearText=function(e){//清除input值
                e.stopPropagation();
                vm.clearBtnHeight=avalon(this).css("height");
                element.value="";
                vm.showClearBtn=false;
                options.clearAfterFun();
            }

            //vm.clearBtn=function(e){
            //    e.stopPropagation();
            //    vm.showClearBtn=false;
            //}
            vm.showClearBtn=false;
            ...
            vm.$init = function(continueScan) {
                ...
                function clearBtnShow(){
                    //var textBox=document.querySelector(".oni-textbox");
                    //var height=avalon(textBox).css("height");
                    //if((height-vm.clearBtnHeight)>0){
                    //    vm.marginClass=(height-vm.clearBtnHeight)/2+"px";
                    //};
                    if(element.value&&options.showClear&&(navigator.appName!="Microsoft Internet Explorer")){
                ...
                avalon.bind(document,"click",function(e){
                    if(e.target==$element[0]) return;  //薛晓峰清除按钮只有点击输入框以外部分才会隐藏
                    e.stopPropagation();
                    vm.showClearBtn=false;
                });
                // 包装原始输入域
    ...
    widget.defaults = {
        ...
        clearAfterFun:function(){//点击后清除按钮响应事件
        },
