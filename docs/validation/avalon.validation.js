...
define(["avalon", "../mmPromise/mmPromise","css!./avalon.validation.css"], function(avalon) {
    ...
    avalon.mix(avalon.duplexHooks, {
        ...
        norequired: {
            ...
            get: function(value, data, next) {
                next(true)
                if(!("placeholder" in document.createElement("input"))&&data.element.getAttribute('placeholder')===value)
                {
                    return ""
                }
                return value
                    ...
                    if (!elem.disabled) {
                        var resolve, reject
                            ...
                            var next = function(a) {
                                ...
                                //如果浏览器不支持placeholder属性value值等于placeholder且不为非空校验时，校验不通过
                                if(!("placeholder" in document.createElement("input"))&&elem.getAttribute('placeholder')===value&&!data.norequired)
                                {
                                    a=false
                                }
                                if (a) {
