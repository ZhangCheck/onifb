...
define([
    ...
    var cnt = 0
    var _getRowNum = function(vmId, field, index, cellValue, rowData, disable) {
        var vmodel = avalon.vmodels[vmId]
        if( vmodel &&  vmodel.pager){
            return (vmodel.pager.currentPage-1)*vmodel.pager.perPages + index+1;
        }
        return index+1;
    };
    function guid() {
    ...
    var widget = avalon.ui.smartgrid = function (element, data, vmodels) {
        ...
        var vmodel = avalon.define(vmId, function (vm) {
            ...
            //选中、切换行的效果
            vm.chooseThisRow=function(){
                avalon(this).toggleClass("oni-smartgrid-row-bgcolor");

            }
            //双击行事件
            vm.dblclickTr=function(){

            }

            //表格cell编辑
            vm.cellEdit=function(){
                //1,
                //$(this).empty().append('<input type="text"  ms-duplex=""> ');

                $(this).find(".oni-smartgrid-column-cell").hide();
                $(this).css({"text-align":"center"}).append('<input type="text" >');
                avalon.scan(element);

                //2,
                //$(this).append('<input type="text" value="aaaa">');
            ...
            vm.sortColumn = function (column, index, event) {
                ...
                if (!vmodel.data.length || !column.sortable)
                    return;
                ...
                if(column.sortTrend=="asc" || column.sortTrend=="ndb"){
                    sortTrend = 'desc';
                }else {
                    sortTrend = 'asc';
                }
                sorting = true;
                vm.sortingColumnIndex = index;
    ...
    function perfectColumns(options, element, vmId) {
        ...
        options.htmlHelper._getRowNum = _getRowNum;
        if(options.showRowNum){
            columns.splice(0,0,{
                ...
                format: "_getRowNum" //包装列数据的方法，此方法名对应到htmlHelper对象中的方法
            })
        }
        for (var i = 0, len = columns.length; i < len; i++) {
