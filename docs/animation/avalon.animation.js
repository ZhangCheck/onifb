...
define(["avalon"], function() {
    ...
    function createTweenImpl(frame, name, value, hidden) {
        ...
        else if(/transform$/i.test(name)){
            parts=[convertTransform(from),convertTransform(value)]
        }else {
        ...
        from = parts[0]
        to = parts[1]
        if(typeof from==="object"&&from!==to){
            tween.start = from
            tween.end = to
            tween.unit = unit
            frame.tweens.push(tween)
        } else if (from + "" !== to + "") { //不处理起止值都一样的样式与属性
            tween.start = from
            tween.end = to
            tween.unit = unit
            frame.tweens.push(tween)
    ...
    function Tween(prop, options) {
        ...
        if(/transform$/i.test(prop)){
            this.update=this.updateTransform
        }
    }
    ...
    Tween.prototype = {
        ...
        updateTransform:function(per,gotoEnd){
            var val={},start,end
            if(this.start.type==="matrix") {
               start=Matrix2Transform(this.start,this.end.type)
                val.type=this.end.type
            }
            else{
                start=this.start
            }
            if(this.end.type==="matrix") {
                end=Matrix2Transform(this.end,this.start.type)
                val.type=this.start.type
            }
            else{
                end=this.end
            }
            ...
            if(gotoEnd){
                ...
                var pos=this.easing(per)
                for(var i=0;i<start.array.length;i++){
                    val.array.push(parseFloat(start.array[i]*1+(end.array[i]-start.array[i])*pos))
                }
            }
            var now=val.type+"("
            for(var i=0;i<val.array.length;i++)
            {
                if(val.type==="rotate"||val.type==="skew"){
                    now+=val.array[i]+"deg,"
                }
                else{
                    now+=val.array[i]+","
    ...
    Tween.propHooks["transform"] = {
            get : function(tween){
                return tween.elem.style.transform||"none"
            },
            set : Tween.propHooks._default.set
    }
    ...
    function convertTransform(val){
        var ret={}
        var args=val.replace("(",",").replace(")","").replace(/deg/g,"").replace("none","").match(avalon.rword) || []
        ret.type=args[0]||"matrix"
        ret.array=new Array()
        for(var i=1;i<args.length;i++){
            ret.array.push(args[i])
        }
        if(ret.array.length<=0)
        {
            ret.array=[1,0,0,1,0,0]
        }
        return ret
    }

    function Matrix2Transform(val,type){
        var ret={}
        ret.type=type
        ret.array=new Array()
        if(val.type===type){
            ...
            switch(type){
                case "rotate":
                    ret.array.push(Math.round(Math.acos(parseFloat(val.array[0]))*180/Math.PI));
                    break;
                case "scale":
                    ret.array.push(parseFloat(val.array[0]));
                    ret.array.push(parseFloat(val.array[3]));
                    break;
                case "skew":
                    ret.array.push(Math.round(Math.atan(parseFloat(val.array[1]))*180/Math.PI));
                    ret.array.push(Math.round(Math.atan(parseFloat(val.array[2]))*180/Math.PI));
                    break;
                case "translate":
                    ret.array.push(parseFloat(val.array[4]));
                    ret.array.push(parseFloat(val.array[5]));
                    break;
                defaut: break;
            }
        }
