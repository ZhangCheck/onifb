...
define(["avalon", "text!./avalon.at.html", "css!../chameleon/oniui-common.css", "css!./avalon.at.css"], function(avalon, template) {
    ...
    function getCaretPosition(ctrl) {
        ...
        /*
         * 自己修改的部分
         * 问题：在ie8下组件不弹提示框并不显示默认值
         * 原因：在ie8下不支持selectionStart属性，导致无法正确获取光标位置
         */
        if (typeof ctrl.selectionStart === "number") {//IE9-或者标准浏览器
            caret = ctrl.selectionStart
        } else if (document.selection) {//IE8-或者支持selection对象的浏览器
            ctrl.focus();
            var r = document.selection.createRange();
            var re = ctrl.createTextRange(), rc = re.duplicate();
            re.moveToBookmark(r.getBookmark());
            rc.setEndPoint('EndToStart', re);
            caret = rc.text.length;
        }
