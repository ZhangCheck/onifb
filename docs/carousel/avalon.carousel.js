...
define(["avalon", "text!./avalon.carousel.html", "css!./avalon.carousel.css", "css!../chameleon/oniui-common.css","../avalon.fb"], function(avalon, template) {
	...
	var widget = avalon.ui.carousel = function(element, data, vmodels) {
		...
		var vmodel = avalon.define(data.carouselId, function(vm) {
			...
			var inited
			vm.$dataAjax = null
			vm.setPictures=function(pictures){
			...
			vm.$init = function(continueScan) {
				...
				var operationFunction=function()
				{
					...
					// 左右滑动切换
					if(options.allowSwipe){
						avalon.bind(element,"swipeleft",function(){
							vm.animate(1);
						})
						avalon.bind(element,"swiperight",function(){
							vm.animate(-1);
						})
					}
				...
				if(options.interface)
				{
					...
					vm.$dataAjax=fb.requestJson(avalon.mix({},options.interface, {
						success: function (interfaceData) {
							...
							{
								vm.pictures=options.dataFormat(interfaceData);
							}
							else
							{
								vm.pictures = interfaceData;
							}
							vm.selections = avalon.range(vm.pictures.length);
							operationFunction();
	...
	widget.defaults = {
		interface:null,//@Object  接口对象
		dataFormat:null,//@function 数据格式化方法
