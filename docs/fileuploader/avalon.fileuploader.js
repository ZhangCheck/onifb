 ...
 */
define(["avalon", "text!./avalon.fileuploader.cus.html", "../browser/avalon.browser", "./eventmixin",
        "./blob",
        "./file",
        "./flasheventhub",
        "./runtime",
    ...
    function (avalon, template, browser, eventMixin, blobConstructor, fileConstructor, fehConstructor, runtimeConstructor, blobqueueConstructor, md5gen, inputProxyContructor) {
        ...
        var widget = avalon.ui[widgetName] = function (element, data, vmodels) {
            ...
            var vmodel = avalon.define(data[widgetName + 'Id'], function (vm) {
                ...
                if(!vm.previews||vm.previews.length==0){
                    vm.previews = [];
                }   // 渲染到Template上的预览数据
                ...
                vm.$fileInputProxy.addEventListener("newFileSelected", function (fileInfo) {
                    ...
                    this.getFileKey(this,fileObj,function (fileKey) {
                        console.log("getFileKey:"+fileObj.name+" "+fileKey);
                        fileObj.fileKey = fileKey;
                        preFileObj.fileKey = fileKey;
                    },this);
                    this.previews.push(preFileObj);
                }, vm);
                ...
                vm.addFileClicked = function (e) {
                    ...
                    if (vm.useHtml5Runtime) {
                        ...
                        //ie10+点击打开文件，不出窗口问题
                        var inputFile=$wrapper.element.querySelector("input");
                        if(inputFile){
                            if (inputFile.type == "file" && inputFile.nodeType == 1) {
                                inputFile.click();
                                return;
                            }
                        }
                        //ie10+点击打开文件，不出窗口问题end

                        //源代码
                        //for (var i = 0; i < $wrapper.element.children.length; i++) {
                        //    var subNode = $wrapper.element.children[i];
                        //    console.log("kk:"+$wrapper.element.innerHTML);
                        //    console.log(subNode);
                ...
                vm.$init = function () {
                    ...
                    element.innerHTML = template.replace(/##PAGE_PATH##/ig,options.swfPath).replace(/##VM_ID##/ig, vm.$id);  // 将vmid附加如flash的url中
        ...
        widget.defaults = {
            head:{
                visible:true,
                content:'上传文件是实时操作，上传、删除会即时同步数据。'
            },
            noPreviewText:'1、添加文件\<br\/\>2、点击上传\<br\/\>3、点击确认',
            previewStageHeight:400,
            swfPath:'',
            ...
            btnGroupVisible: true, //@config {Boolean} 是否是显示‘添加’‘上传’按钮
            ...
            localFileKeyGen: function (opts, fileObj, resolve, reject) {
                var key = fileObj.name + "#" + fileObj.size + "#" + fileObj.fileLocalToken + "#" +(new Date()-0);//
                //console.log("localFileKeyGen  :"+key+" key:"+opts.$md5gen(key));
                resolve(opts.$md5gen(key));
            },
            ...
            onFileStatusChanged: function (fileObj, beforeStatus) {
                ...
                console.log("onFileStatusChanged:"+fileObj.fileKey+" "+fileObj.status+" "+fileObj.fileLocalToken);
                if (fileObj.status == fileObj.FILE_UPLOADED) {
                    previewVm.uploadProgress = 100;
                    previewVm.done = true;
                    this.onFileUploadSuccess(fileObj);
                } else if (fileObj.status == fileObj.FILE_ERROR_FAIL_UPLOAD) {
                    console.log("this.onFileUploadFailure:" + fileObj.status + " " + fileObj.fileLocalToken+" "+fileObj.fileKey);
                    this.onFileUploadFailure(fileObj);
                }
            ...
            /**
             * 上传上传失败事件
             * add by wolf.j on 2015-11-18
             * @param fileObj
             */
            onFileUploadFailure: function (fileObj) {
                console.log("default.onFileUploadFailure:" + fileObj.status + " " + fileObj.fileLocalToken);
            },
            /**
             * 上传上传成功事件
             * add by wolf.j on 2015-11-18
             * @param fileObj
             */
            onFileUploadSuccess: function (fileObj) {
                console.log("default.onFileUploadSuccess:" + fileObj.status + " " + fileObj.fileLocalToken);
            },
