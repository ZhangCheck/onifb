...
*/
	blobQueue.prototype.taskFn = function () {
		var me = this;
		//console.log("taskFn PoolFull:"+!this.isRequestPoolFull()+" ln:"+this.queue.length+" inSending:"+this.inSending);
		while (!this.isRequestPoolFull() && this.queue.length != 0) {
			this.inSending++;	// 正在发送+1
			console.log("taskFn inSending:"+this.inSending+" ");
			var blob = this.pop();
	...
	blobQueue.prototype.onBlobSuccess = function (blob, responseText, textStatus) {
		this.inSending--;
		avalon.log("****FileUploader.blobqueue: Blob tranfered. File token: ", blob.fileObj.fileLocalToken, ". Blob Index: ", blob.index);
	...
	blobQueue.prototype.onBlobError = function (blob, textStatus, error) {
		...
		blob.fileObj.responseText = textStatus;
