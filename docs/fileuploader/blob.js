...
define(["avalon"], function (avalon) {
	...
	blob.prototype.onSuccess = function (response, textStatus, promise) {
		...
		if(promise){
			var responseText = promise.responseText;
		}else{
		...
		this.dispatchEvent("blobUploaded", this, responseText, textStatus);
	}

	blob.prototype.onError = function (responseText, textStatus, error) {
		this.retried++;
		...
		if (this.retried < retryConfig) {
			...
			delete this.uploadConfig;
			this.unbindFlashEventHub();
			this.dispatchEvent("blobErrored", this, responseText);
		}
	}
	...
	blob.prototype.__html5upload = function (config) {
		...
		var requestConfig = {
		    ...
		    error: function (promise, textStatus, error) {
		    	me.onError(promise.responseText, textStatus, error);
		    }
		};
