...
require(["fb_lib/avalon.fb","fb_lib/dropdown/avalon.dropdown","fb_lib/simplegrid/avalon.simplegrid","fb_lib/mobile/avalon.mobile","css!../css/pageList","domReady!"],function(){
    var df = avalon.cookie.get("fbMode",16),currentPageTemp
    avalon.cookie.set("fbMode",df)
    ...
    var vm = avalon.define({
        ...
        currentPage:0,
        $pages:[
            {name:"首页",url:"index.html",author:"作者",state:""}
        ],
        $pagesOpt: {
        ...
        $projectModuleOpt:{
            ...
            data:[
                {name:"模块名",fun:"模块功能",api:"",path:"",author:"作者",state:""}
            ]
        },
        $dataSourceOpt:{
            data:[
            ...
            onChange: function() {
                ...
                vm.clickMobilePage(currentPageTemp);
            }
        },
        $projectStructureOpt:{
        ...
        clickMobilePage:function(index){
            currentPageTemp = vm.currentPage = index
            var url = vm.$pages[index].url
            avalon.vmodels["mobile1"].src = url+(url.indexOf("?")>-1?"&":"?")+"fbMode="+vm.fbMode
        }
    });
    vm.$pagesOpt.data = vm.$pages;
    avalon.scan();
    $('body').show();
    vm.clickMobilePage(0)
})